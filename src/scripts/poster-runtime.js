...
const WEATHER_CODES = {
  ...
  61: 'Lluvia débil', 63: 'Lluvia', 65: 'Lluvia fuerte', 71: 'Nieve débil', 73: 'Nieve', 75: 'Nieve fuerte',
  80: 'Chubascos débiles', 81: 'Chubascos', 82: 'Chubascos fuertes', 95: 'Tormenta', 96: 'Tormenta con granizo', 99: 'Tormenta fuerte',
};
...
function loadConfig() {
  ...
  try { return JSON.parse(saved); } catch { return cloneDefaultPoster(); }
}
...
function widgetDefaults(type) {
  ...
  if (type === 'text') return { ...base, text: 'Texto libre', color: '#ffffff', fontSize: 42, fontFamily: 'system-ui', bold: true, italic: false, letterSpacing: 0 };
  ...
  if (type === 'image') return { ...base, imageUrl: 'https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=900&auto=format', fit: 'cover' };
...
function textStyle(widget) {
  return `color:${widget.color || '#fff'};font-size:${widget.fontSize || 32}px;font-family:${widget.fontFamily || 'system-ui'};font-weight:${widget.bold ? 800 : 400};font-style:${widget.italic ? 'italic' : 'normal'};letter-spacing:${widget.letterSpacing || 0}px;`;
}
...
async function fetchWeather(place, forecast = false) {
  ...
  const geo = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(name)}&count=1&language=es&format=json`).then((r) => r.json());
  const hit = geo.results?.[0];
  if (!hit) throw new Error('Municipio no encontrado');
  const params = forecast ? 'daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=7' : 'current=temperature_2m,weather_code,wind_speed_10m';
  const data = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${hit.latitude}&longitude=${hit.longitude}&${params}&timezone=auto`).then((r) => r.json());
  return { name: hit.name, data };
}

function renderWeather(widget, target) {
  target.textContent = 'Cargando tiempo…';
  fetchWeather(widget.municipality).then(({ name, data }) => {
  ...
  }).catch(() => { target.textContent = 'No se pudo cargar el tiempo'; });
}

function renderForecast(widget, target) {
  target.textContent = 'Cargando previsión…';
  fetchWeather(widget.municipality, true).then(({ name, data }) => {
    const rows = data.daily.time.slice(0, Number(widget.days || 4)).map((day, index) => `<li><b>${new Date(day).toLocaleDateString('es-ES', { weekday: 'short' })}</b><span>${Math.round(data.daily.temperature_2m_min[index])}° / ${Math.round(data.daily.temperature_2m_max[index])}°</span><small>${WEATHER_CODES[data.daily.weather_code[index]] || ''}</small></li>`).join('');
    target.innerHTML = `<strong>${escapeHtml(name)}</strong><ul>${rows}</ul>`;
  }).catch(() => { target.textContent = 'No se pudo cargar la previsión'; });
}
...
function renderWidget(widget, editable = false) {
  ...
  if (widget.type === 'text') body.innerHTML = `<div style="${textStyle(widget)}">${escapeHtml(widget.text)}</div>`;
...
function initViewer(root) {
  ...
  setInterval(() => screen.querySelectorAll('time').forEach((node) => {
    const widget = config.pages.flatMap((page) => page.widgets).find((item) => node.closest('[data-id]')?.dataset.id === item.id);
...
function initBuilder(root) {
  ...
  function renderPages() {
    pageList.innerHTML = config.pages.map((page, index) => `<button type="button" class="${index === selectedPageIndex ? 'is-active' : ''}" data-page="${index}">${escapeHtml(page.name)}</button>`).join('');
    pageList.querySelectorAll('button').forEach((button) => button.addEventListener('click', () => { selectedPageIndex = Number(button.dataset.page); selectedWidgetId = currentPage().widgets[0]?.id || null; sync(); }));
  }
  ...
  function renderInspector() {
    ...
    if (!widget) { inspector.innerHTML += '<p>Selecciona o añade un widget.</p>'; return; }
    ...
    inspector.querySelectorAll('select').forEach((select) => { if (widget[select.dataset.key]) select.value = widget[select.dataset.key]; });
  }
  ...
  root.querySelector('[data-reset]').addEventListener('click', () => { config = cloneDefaultPoster(); selectedPageIndex = 0; selectedWidgetId = config.pages[0].widgets[0]?.id; sync(); });
  root.querySelector('[data-export]').addEventListener('click', () => root.querySelector('[data-json]').value = JSON.stringify(config, null, 2));
  root.querySelector('[data-import]').addEventListener('click', () => { try { config = JSON.parse(root.querySelector('[data-json]').value); selectedPageIndex = 0; selectedWidgetId = config.pages[0]?.widgets[0]?.id || null; sync(); } catch { alert('JSON no válido'); } });

  function moveWidget(event, mode) {
    const widget = currentPage().widgets.find((item) => item.id === event.currentTarget.dataset.id);
    if (!widget) return;
    ...
    const onMove = (move) => {
      ...
      if (mode === 'resize') { widget.w = clamp(start.widget.w + dx, 5, 100 - widget.x); widget.h = clamp(start.widget.h + dy, 5, 100 - widget.y); }
      else { widget.x = clamp(start.widget.x + dx, 0, 100 - widget.w); widget.y = clamp(start.widget.y + dy, 0, 100 - widget.h); }
      selectedWidgetId = widget.id;
      sync();
    };
    addEventListener('pointermove', onMove);
    addEventListener('pointerup', () => removeEventListener('pointermove', onMove), { once: true });
  }
  function startDrag(event) { if (!event.target.closest('.widget-resize')) moveWidget(event, 'drag'); }
  function startResize(event) { moveWidget(event, 'resize'); }
  sync();
}
